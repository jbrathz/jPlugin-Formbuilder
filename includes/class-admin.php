...
final class Admin {
	...
	public function register_settings(): void {
		add_filter( 'option_page_capability_jfb_settings_group', static fn(): string => 'jfb_manage_settings' );
	...
	public function settings_page(): void {
		$this->guard( 'jfb_manage_settings' );
		$settings        = Settings::get();
		$test_status     = sanitize_key( wp_unslash( (string) ( $_GET['jfb_test'] ?? '' ) ) );
		$settings_saved  = '1' === sanitize_text_field( wp_unslash( (string) ( $_GET['jfb_saved'] ?? '' ) ) );
		$test_message    = '';
		$keys_overridden = defined( 'JFB_TURNSTILE_SITE_KEY' ) || defined( 'JFB_TURNSTILE_SECRET_KEY' );

		if ( $test_status ) {
			$test_result = get_transient( Security::turnstile_test_transient_key( get_current_user_id() ) );
			if ( is_array( $test_result ) && $test_status === ( $test_result['status'] ?? '' ) ) {
				$test_message = sanitize_text_field( (string) ( $test_result['message'] ?? '' ) );
				delete_transient( Security::turnstile_test_transient_key( get_current_user_id() ) );
			}
			if ( '' === $test_message ) {
				$test_message = 'ok' === $test_status ? __( 'Cloudflare accepted the saved secret key.', 'jplugin-formbuilder' ) : __( 'Cloudflare connection test failed.', 'jplugin-formbuilder' );
			}
		}
		?>
		<div class="wrap jfb-admin-wrap"><?php $this->masthead( __( 'Security & appearance', 'jplugin-formbuilder' ), __( 'Keep the public surface small, predictable, and easy to audit.', 'jplugin-formbuilder' ) ); ?><?php if ( $settings_saved ) : ?><div class="notice notice-success is-dismissible"><p><?php esc_html_e( 'Settings saved.', 'jplugin-formbuilder' ); ?></p></div><?php endif; ?><?php if ( $test_status ) : ?><div class="notice <?php echo 'ok' === $test_status ? 'notice-success' : 'notice-error'; ?>"><p><?php echo esc_html( $test_message ); ?></p></div><?php endif; ?>
			<form method="post" action="<?php echo esc_url( wp_make_link_relative( admin_url( 'admin-post.php' ) ) ); ?>" class="jfb-settings-grid"><input type="hidden" name="action" value="jfb_save_settings"><?php wp_nonce_field( 'jfb_save_settings' ); ?>
				<section class="jfb-panel"><span class="jfb-kicker">Cloudflare</span><h2><?php esc_html_e( 'Turnstile & rate limiting', 'jplugin-formbuilder' ); ?></h2><?php if ( $keys_overridden ) : ?><div class="notice notice-warning inline"><p><?php esc_html_e( 'Turnstile runtime keys are overridden by constants in wp-config.php. Saved database keys will not be used until those constants are removed.', 'jplugin-formbuilder' ); ?></p></div><?php endif; ?><label class="jfb-switch"><input type="checkbox" name="jfb_settings[turnstile_enabled]" value="1" <?php checked( $settings['turnstile_enabled'] ); ?>><span><?php esc_html_e( 'Require Turnstile on public forms', 'jplugin-formbuilder' ); ?></span></label><label><?php esc_html_e( 'Site key', 'jplugin-formbuilder' ); ?><input type="text" name="jfb_settings[turnstile_site_key]" value="<?php echo esc_attr( $settings['turnstile_site_key'] ); ?>" autocomplete="off"></label><label><?php esc_html_e( 'Secret key', 'jplugin-formbuilder' ); ?><input type="password" name="jfb_settings[turnstile_secret_key]" value="" placeholder="<?php echo esc_attr( $settings['turnstile_secret_key'] ? __( 'Saved - leave blank to keep it', 'jplugin-formbuilder' ) : __( 'Enter secret key', 'jplugin-formbuilder' ) ); ?>" autocomplete="new-password"><small><?php echo $settings['turnstile_secret_key'] ? esc_html__( 'A secret key is saved. Enter a new value only to replace it.', 'jplugin-formbuilder' ) : esc_html__( 'No secret key is saved yet.', 'jplugin-formbuilder' ); ?></small></label><div class="jfb-inline-fields"><label><?php esc_html_e( 'Attempts / 10 min', 'jplugin-formbuilder' ); ?><input type="number" min="1" max="100" name="jfb_settings[rate_limit]" value="<?php echo esc_attr( $settings['rate_limit'] ); ?>"></label><label><?php esc_html_e( 'Minimum fill time', 'jplugin-formbuilder' ); ?><input type="number" min="1" max="30" name="jfb_settings[min_fill_seconds]" value="<?php echo esc_attr( $settings['min_fill_seconds'] ); ?>"></label></div><p><button type="submit" class="button" name="jfb_intent" value="save_test"><?php esc_html_e( 'Save and test Cloudflare keys', 'jplugin-formbuilder' ); ?></button></p><p class="description"><?php esc_html_e( 'Application limits complement Cloudflare WAF; they do not replace edge protection.', 'jplugin-formbuilder' ); ?></p></section>
			...
			<section class="jfb-panel jfb-data-panel"><span class="jfb-kicker"><?php esc_html_e( 'Data & Privacy', 'jplugin-formbuilder' ); ?></span><h2><?php esc_html_e( 'Uninstall keeps everything', 'jplugin-formbuilder' ); ?></h2><p><?php esc_html_e( 'Removing the plugin will not delete forms, submissions, settings, or files.', 'jplugin-formbuilder' ); ?></p><code><?php echo esc_html( implode( ', ', array( Database::table( 'forms' ), Database::table( 'submissions' ), Database::table( 'submission_files' ), Database::table( 'rate_limits' ) ) ) ); ?></code><code><?php echo esc_html( Settings::OPTION ); ?></code><code><?php echo esc_html( Storage::base_dir() ); ?></code><p><strong><?php echo Storage::is_protected() ? esc_html__( 'Private vault: writable and protected', 'jplugin-formbuilder' ) : esc_html__( 'Private vault needs attention before file forms can be published', 'jplugin-formbuilder' ); ?></strong></p></section>
				<p class="submit"><button type="submit" class="button button-primary button-hero" name="jfb_intent" value="save"><?php esc_html_e( 'Save settings', 'jplugin-formbuilder' ); ?></button></p></form>
		</div>
		<?php
	}
